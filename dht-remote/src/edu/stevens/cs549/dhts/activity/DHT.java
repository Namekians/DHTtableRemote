...
public class DHT extends DHTBase implements IDHTResource, IDHTNode, IDHTBackground {
	...
	private NodeInfo getSucc(NodeInfo info) throws Failed {
		...
		if (localInfo.addr.equals(info.addr)) {
			...
			try {
//				Document doc = getResource(info.addr + RestKeyIDforURI.GET_SUCC);
//				// getResouce is a wrapper method to perform GET service call.
//				int succ_id = Integer.parseInt(doc.getElementsByTagName("id").item(0).getTextContent());
//				String succ_addr = doc.getElementsByTagName("addr").item(0).getTextContent();
//				NodeInfo succ = new NodeInfo(succ_id, new URI(succ_addr));
//				return succ;
				return client.getRequest(new URI(info.addr + "/succ")).readEntity(NodeInfo.class);
			} catch (Exception e) {
				throw new Failed(String.format("getSucc of node %s failed!", info.id));
			}
