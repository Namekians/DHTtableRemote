...
public class SessionManager {
	...
	public void acceptSession() throws IOException {
		...
		try {
			 ...
			 */
			Session session=currentServer.getSession();
			LocalShell ls=LocalShell.
					createRemotelyControlled(ShellManager.getShellManager().getCurrentShell().getLocal(),
							new ProxyContext(session.getBasicRemote()));
	...
	public void rejectSession() {
		...
		try {
			...
			CloseReason cr=new CloseReason(CloseCodes.CANNOT_ACCEPT,"Reject");
			try {
				currentServer.getSession().close(cr);
			} catch (IOException e) {
	...
	public void closeCurrentSession() {
		...
		try {
			...
			CloseReason cr=new CloseReason(CloseCodes.NORMAL_CLOSURE,"Normal closure");
			try {
				currentServer.getSession().close(cr);
			} catch (IOException e) {
				// TODO Auto-generated catch block
